...
let ``ToString should return proper stringify version of the RecurringEvent`` () =
    ...
    Assert.AreEqual("test year 2: it's a test year 2 (Every year on Thirteenth of June)", result[2])
...
let ``update should return updated version of the RecurringEvent array if Event is found, based on Id, or return the same version`` () =
    ...
    Assert.AreEqual("test year 2", notUpdatedResult[2].Name)
    Assert.AreEqual("test week 2 DELETED", notUpdatedResult[3].Name)

[<Test>]
let ``generateEvents should return the correct Events from the RecurringEvents`` () =
    let maxCount = 5
    let result = RecurringEvent.generateEvents maxCount now events
    ...
    Assert.AreEqual("test year 2", result[3].Name)
...
let ``getViewable should return proper RecurringEvents`` () =
    ...
    Assert.AreEqual("test year 2", result[1].Name)
    Assert.AreEqual("test week 2", result[2].Name)
